        Subroutine  X0h1 (ad,wv,code,rho,nref,h,k,l,ising,
     *                    mode_x0h,jpr,QB,xr0,xi0,
...
c h(nref),
c k(nref),
c l(nref) -     Bragg reflections indices. If zero hkl are passed for a
c               reflection, then X0H requests them from terminal. For
c               hexagonal and trigonal structures the indices need to
c               be in orthogonal settings. At exit all indices are
c               defined.
...
c xrf(nref),
c xif(nref),
c xqf(nref) -   modula of the real and imaginary parts of dipole
...
c This is not passed anywhere, just to store between x0h1 calls:
...
c X0H can handle crystals with the number of components not
...
c (returned via common/back1/ and common/back2/)         |
     ...
     *        mode.eq.md_NewCrystal_a) Then !--+         |
            ...
            if (ifail.eq.0)  Call DBacklat()  !|         |this calculates vol
                                                        ...
                                                        !|
          if (lbev .ne.1) stop 'X0h1: no back lattice'  !|
                                                        !|
        endif  !-----------------------------------------+
...
c (1.e+24) is the recalculation factor from cm^3 to A^3:
        if (isym.ne.8)  Then  !------------------------------------------+
          if (abs(vol).lt.1.E-37) stop 'X0h1: crystal cell vol=0'       !|
  ...
  113       format(' X0h1: Incorrect or unspecified rho=',g10.3/
     ...
     *      ' Otherwise, please input correct rho parameter.')   !|      |
          ...
          if (abs(rho).lt.1.E-37) stop 'X0h1: amorphous material rho=0' !|
...
c +--------------------------------------------------+    |
c | If HKL indices are not given, then request them: |    |
c +--------------------------------------------------+    |
          if (ind(1)    .eq.0   .AND.                    !|
     *        ind(2)    .eq.0   .AND.                    !|
     *        ind(inmax).eq.0)          Then  !--------+  |
     ...
     *      write (txt(1)(12:13),'(i2)') iref         !|  |
            ...
            goto 998                                  !|  |
                                                         ...
                                                         !|
c +----------------------------------------------+        |
c | Calculate Bragg angle; if QB>90, then:       |        |
c | -- in interactive mode, take QB=90 and warn, |        |
c | -- in batch mode,       take QB=0 and warn   |        |
c +----------------------------------------------+        |
...
c Do not calculate sin,cos,... (for speed):               |
        isico = 0                                        !|
        QB8 = Bragan8 (wave,ind,inmax,iwarn,d8,          !|
     *                          isico,s,s,s,s,s,s)       !|
        QB(iref) = Sngl(QB8)                             !|
        d        = Sngl(d8)
        if (abs(QB(iref)).lt.1.E-37) Then !----------+    |
          ifail=5  !the length of message in Bragan  |    |
          goto 998                                  !|    |
        endif !--------------------------------------+    |
                                                         !|
c This is equivalent sin(QB)/wavelength:                  |
          s = 1./(2.*d)                                  !|
                                                         !|
          xrhc = (0.,0.)                                 !|
          xihd = (0.,0.)                                 !|
          xihq = (0.,0.)                                 !|
c +------------------------------+                        |
c | Calculate Xh by summing over |                        |
...
c +------------------------------------------------+   |  |
c | Calculate the scattering factor for reflecting |   |  |
c | X-rays by the Bragg angle QB for the i-th crys-+   |  |
c | crystal component with account for dispersion  |   |  |
c | corrections:                                   |   |  |
c +------------------------------------------------+   |  |
                                                      !|  |
            ...
            if (ifail.ne.0)     goto 998              !|  |
c +--------------------------------------+             |  |
c | Summ the exponential factors of each |             |  |
c | element contribution:                |             |  |
c +--------------------------------------+             |  |
                                                      !|  |
            sumexp = (0.,0.)                          !|  |
            do      j=1,kol(indx)  !=============+     |  |
              sum = h(iref)*wx(indx,j)          !|     |  |
     +            + k(iref)*wy(indx,j)          !|     |  |
     +            + l(iref)*wz(indx,j)          !|     |  |
              sumexp = sumexp +                 !|     |  |
     +                  exp(2.*pi*im*sum*ipm)   !|     |  |
...
c ..................                                   |  |
            if (ipr.eq.2)   Then   !--------------+    |  |
              write   (txt,413)  name(indx),     !|    |  |
     *                           abs(sumexp)     !v    v  v
  413         format  ('Element: ',a,
     *        '    Sum of exponents =',g10.3)    !^    ^  ^
              Call    Priblo  (txt,1,0,0,0,ifa)  !|    |  |
            endif  !------------------------------+    |  |
          ...
          enddo   !====================================+  |
                                                         !|
c ------------------                                      |
c The module of xrh,xih,xqh:                              |
          xrh(iref) = abs(xrhc)                          !|
          xih(iref) = abs(xihd)                          !|
          xqh(iref) = abs(xihq)                          !|
                                                         !|
c The phase of xrh,xih,xqh:                               |
          if (abs(xrhc).lt.1.E-37) Then  !--------------+ |
            xrf(iref)=0.                               !| |
          else  !---------------------------------------+ |
            xrf(iref)=Atan2(aimag(xrhc),real(xrhc))/pi !| |
          endif  !--------------------------------------+ |
                                                         !|
          if (abs(xihd).lt.1.E-37) Then  !--------------+ |
            xif(iref)=0.                               !| |
          else  !---------------------------------------+ |
            xif(iref)=Atan2(aimag(xihd),real(xihd))/pi !| |
          endif  !--------------------------------------+ |
                                                         !|
          if (abs(xihq).lt.1.E-37) Then  !--------------+ |
            xqf(iref)=0.                               !| |
          else   !--------------------------------------+ |
            xqf(iref)=Atan2(aimag(xihq),real(xihq))/pi !| |
          endif  !--------------------------------------+ |
                                                         !|
          if (ipr.ne.0)   Then  !-----------------------+ |
            write  (txt,18) QB(iref),                  !| |
     *                  xrh(iref),xrf(iref),           !| |
     *                  xih(iref),xif(iref),           !| |
     *                  xqh(iref),xqf(iref)            !| |
...
c |Reflex indices:                   |Bragg angle:            |

  18    format  (
     *  '+',34('-'),                       '-',24('-'),             '+'/
     *  '|Reflection:',23x,                '|Bragg angle:',g12.4,   '|'/
     *  '|',17('-'),      '-',16('-'),     '-----',20('-'),         '|'/
     *  '|',17x,          '|',7x,'Module',7x,  '|',6x,'Phase/pi',6x,'|'/
  ...
  998   continue
        if (ipr.eq.2 .AND. linesX0hWarn.gt.0)  Then  !-----------------+
          Call Priblo("X0h1: warnings summary for "//code,1,1,0,0,ifa)!|
