...
c                                         trds_97
c
c *** Version 1.1 ***  -- Printout for wavefields with simultaneous
c                         switching the roughness transition layers
c                         off.
c *** Version 1.0 ***  -- Interface roughness /transition layers
c                         included.
c
c +=====================================================+
...
c####################################################### 1
        ...
        Subroutine  MMRE99 (f0, f0_, E0, Es, R0s, Phase_shift,
...
cc      Parameter       (N_Tran_Max    = 101)           !50
     ...
     *             N_Plus = N_Fields/2)
        ...
        Complex*16 xn_ij(3,3), xn1_ij(3,3),         ! normalized p-z-s matrix
     *             E0(2), Es(2),                    ! incident,reflected waves (sigma & pi)
     *             Dy(N_Roots, N_TT_Max),           ! sigma-Fields in layers
     *             Dz(N_Roots, N_TT_Max),           ! pi-Fields in layers
     *             Dx(N_Roots, N_TT_Max),           ! longitudinal Fields in layers
     ...
     *             i, j, k                          ! work sell
        ...
        Complex*16 x_ij(3,3,N_TT_Max)               ! polarizabilities matrix
...
c--------------------------------------------------------
        ...
        do      i=1,N_Fields  !=======+
        do      k=1,N_TT      !=====+ |
          Dy(i,k) = (0.D0, 0.D0)   !| |D_sigma
          Dz(i,k) = (0.D0, 0.D0)   !| |D_pi
          Dx(i,k) = (0.D0, 0.D0)   !| |D_k
          u (i,k) = (0.D0, 0.D0)   !| |
          F (i,k) = (1.D0, 0.D0)   !| |
        enddo  !====================+ |
        enddo  !======================+
...
c and magnetic_2=2. Hopefully, it is not physical!        | |
     ...
     *                      Magnetic(N_Used+1))          !| |
            Call   Transi_rM99(Thickness_Tr(N_Used),     !| |
...
c Substrate fields (T_n = W_tt^n * T_0):
        Dy(1,N_Used+1) = W_tt(1,1,N_Used)*Dy(1,1)/F(1,N_Used+1)  !(F=1)
     +                 + W_tt(1,2,N_Used)*Dy(2,1)/F(2,N_Used+1)  !(F=1)
        Dy(2,N_Used+1) = W_tt(2,1,N_Used)*Dy(1,1)/F(1,N_Used+1)  !(F=1)
     +                 + W_tt(2,2,N_Used)*Dy(2,1)/F(2,N_Used+1)  !(F=1)
        Dy(3,N_Used+1) = (0.0D0, 0.0D0)
        Dy(4,N_Used+1) = (0.0D0, 0.0D0)
c The rest fields:
c R_k = (1-M_rt^{k+1}*W_tr^k)^{-1} * (M_rr^{k+1}*R_{k+1) + M_rt^{k+1}*W_tt^k*T_0)
c T_k = W_tt^k*T_0 + W_tr^k*R_k
        do      k = N_Used-1,1,-1   !===========+
          Call  Find_Fields (Dy(1,1),          !|T_0
     *                       Dy(N_Plus+1,k+2), !|R_{k+1}
     *                       Dy(1,k+1),        !|T_k
     *                       Dy(N_Plus+1,k+1), !|R_k
     *                       W_tt(1,1,k),      !|
     *                       W_tr(1,1,k),      !|
...
c       iDebug=1
        if (iDebug.ne.0)        Then !---------------------+
          write (3,50)  g0                                !|
  50      format('The Dy-wavefields for g0/TERang=',g9.3) !|
                                                          !|
          do    k = 1, N_Used  !====================+      |
            if (k.lt.N_Used)  Then !---+            |      |
              j = N_Fields            !|            |      |
            ...
            write (3,55) k,(Abs(Dy(i,k+1)),i=1,j)  !|      | !!!!!!! DEBUG
...
c is along Y or Z, we do not see it!                   | TO BE FIXED!!!!!!
     ...
     *     (abs(x_ij(2,3,k)) .gt. (1.E-20)) .AND.     !|
     *     (abs(x_ij(3,2,k)) .gt. (1.E-20))) Then !--+ |
                                                    !| |
     ...
     *                           xn_ij, xabs, 3, 3) !| |
            ...
            do      i=1,N_Fields !==============+    | |
              V      = u(i,k)                  !|    | |
c This needs clarification -- not rtested yet!  |    | |
c -- as of 2003/07/02                           |    | | TO CLARIFY !!!!!!
c             V      = (V**2-g0**2-xn_ij(1,1)) !|    | |
c    /               /  xn_ij(1,2)             !|    | |
              V      = (V**2-g0**2-xn_ij(2,2)) !|    | |
     /               /  xn_ij(2,3)             !|    | |
              Dz(i,k)= V * Dy(i,k)             !|    | |
            enddo  !============================+    | |
            if (iDebug.ne.0)  Then !-----------+     | |
              write (3,51)                    !|     | |
  51          format('The Dz-wavefields:')    !|     | |
              write (3,55) k-1,(Abs(Dz(i,k)), !|     | | !!!!!!!!!!! DEBUG
     *                     i=1,N_Fields)      !|     | | !!!!!!!!!!! DEBUG
            endif  !---------------------------+     | |
          else  !------------------------------------| |
            Dz(1,k) =(0.,0.)                        !| |
...
c    *                    N_Fields, N_Fact(k), i_Born)!|
  ...
  100   continue
c ifail=1,2,3    dispersion roots problems (c02adf)
c ifail=11,12,13 dispersion roots problems (c02adf)
c ifail=100  Dispersion_Roots: Count of Im(u) # 2
c ifail=121  Smatrix_M: det=0 for magnetic=1
c ifail=122  Smatrix_M: det=0 for magnetic=2
...
c####################################################### 1b
        ...
        Subroutine  Process_Layer_M (g0, gx, TERang,
     *                               x_ij, xabs, k0,
     *                               Depth, Magnetic,
     *                               S, B, u, F,
     *                               N_Fields, ifail)
...
c +===============================+
     ...
     *                  N_Fields
        Parameter      (N_Msize = 4)
        Complex*16      x_ij(3,3),                 ! normalized polariz.
...
c####################################################### 2b
        ...
        Subroutine  Smatrix_M (g0, gx, TERang,
     *                         x_ij, xabs,
     *                         Magnetic,
     *                         N_Fields, S, u, ifail)
c +-------------------------------+
...
c Find dispersion equation roots:
        Call    Dispersion_Roots_M (g0, gx, u, x_ij, xabs, TERang,
     *                              sy, sz, xx, zz, delta,
...
c Magnetic layer:                                             |
c---------------                                              |
c (these are calculated in Dispersion_Roots_M):               |
c         sy = g0*g0 + x_ij(2,2)                              |
c         sz = g0*g0 + x_ij(3,3)                              |
c         xx = 1. + x_ij(1,1)*xabs                            |
c         zz = 1. + x_ij(3,3)*xabs                            |
          do    j=1,N_Fields  !============================+  |
            uu  = u(j)*TERang                             !|  |
c (gx is not normalized!)                                  |  |
            unx = uu*gx                                   !|  |
c                                                          |  |
            det = (x_ij(1,3)*xabs+unx)                    !|  |
     *          * (x_ij(3,1)*xabs+unx)                    !|  |
     -          - (xx-uu*uu)*sz*xabs                      !|  |
            if (abs(det) .lt. (1.E-20)) Then !-----+       |  |
              write (3,*) ' Smatrix_M: ',         !|       |  |
     *                    'det=0 for magnetic=1'  !|       |  |
              ifail = 121                         !|       |  |
              return                              !|       |  |
            endif  !-------------------------------+       |  |
            det = xabs / det                              !|  |
c                                                          |  |
            Px  = x_ij(1,2)*sz*xabs                       !|  |
     -          - x_ij(3,2)*(x_ij(1,3)*xabs+unx)          !|  |
c                                                          |  |
            Pz  = x_ij(3,2)*(xx-uu*uu)                    !|  |
     -          - x_ij(1,2)*(x_ij(3,1)*xabs+unx)          !|  |
c                                                          |  |
            Px  = Px*det                                  !|  |
            Pz  = Pz*det                                  !|  |
            V   = uu*Px - gx*Pz                           !|  |
            ...
            S(4,j) = W                                    !|  |  | w, w, w, w |
                                                          !|  |
c           write (3,1) "S-matrix:", (S(i,j),i=1,4)       !|  |
c 1         format(1x,a,3x,                               !|  |
c    *             10(' (',g11.5,',',g11.5,') ',:))       !|  |
          enddo  !=========================================+  |
                                                             !|
        elseif (Magnetic.eq.2) Then !-------------------------|
                                                             !|
c Special case of magnetic layer                              |
c (M||Y, de-coupled sigma & pi)                               |
c-------------------------------                              |
c (these are calculated in Dispersion_Roots_M):               |
c         sy    = g0*g0 + x_ij(2,2)                           |
c         sz    = g0*g0 + x_ij(3,3)                           |
c         xx    = 1. + x_ij(1,1)*xabs                         |
c         delta = -xabs*x_ij(1,3)*x_ij(3,1)/xx                |
c----------                                                   |
          det   = gx*gx - delta*xabs                         !|
          if (abs(det) .lt. (1.E-20)) Then !-----+            |
            write (3,*) ' Smatrix_M: ',         !|            |
     *                  'det=0 for magnetic=2'  !|            |
            ifail = 122                         !|            |
            return                              !|            |
          endif  !-------------------------------+            |
          do      j=1,N_Fields  !========================+    |
...
c (gx is not normalized!)                            |   |    |
              unx = uu*gx                           !|   |    |
              Rx  = - (unx+x_ij(1,3)*xabs)/det      !|   |    |
c             V   = - (uu*x_ij(1,3)*xabs+gx*xx)/det !|   |    |
              V   = uu*Rx - gx                      !|   |    |
              W   = Rx/TERang                       !|   |    |
            else  !----------------------------------|   |    |
              V   = 0.                              !|   |    |
...
c####################################################### 3b
        ...
        Subroutine  Dispersion_Roots_M (g0, gx, u, x_ij, xabs, TERang,
     *                                  sy, sz, xx, zz, delta,
     *                                  Magnetic, N_Fields, ifail)
c +-------------------------------+
c |For MAGNETIC x-ray reflection. |
...
c------------------------------------------------------
     ...
     *             delta
        ...
        Real*8     g0,                             ! normalized incid.angle g0=f0/sqrt(x0)
     *             gx,                             ! qx=sqrt(1-f0**2)
     *             gg,                             ! qx=sqrt(1-f0**2)*sqrt(x0)
     *             ur(N_Msize+1), ui(N_Msize+1)
...
c---------------                                                     |
          ...
          xx = 1. + x_ij(1,1)*xabs                                  !|
          zz = 1. + x_ij(3,3)*xabs                                  !|
c The terms Q(2) and Q(4) are smaller than the others by sqrt(x_ij): |
          gg = gx*TERang                                            !|
          Q(1) = zz                                                 !|
          Q(2) = gg*(x_ij(1,3)+x_ij(3,1))                           !|
          Q(3) = (x_ij(1,3)*x_ij(3,1) + x_ij(2,3)*x_ij(3,2))*xabs   !|
     -         - zz*sy - xx*sz                                      !|
          Q(4) = gg*(x_ij(1,2)*x_ij(2,3) + x_ij(2,1)*x_ij(3,2)      !|
     -         -     (x_ij(1,3)+x_ij(3,1))*sy)                      !|
          Q(5) = xx*(sy*sz-x_ij(2,3)*x_ij(3,2))                     !|
     +         + (x_ij(1,2)*x_ij(3,1)*x_ij(2,3)                     !|
     +         +  x_ij(2,1)*x_ij(1,3)*x_ij(3,2)                     !|
     -         -  x_ij(1,2)*x_ij(2,1)*sz                            !|
     -         -  x_ij(1,3)*x_ij(3,1)*sy)*xabs                      !|
                                                                    !|
c+--------------------------+                                       !|
c|Find the dispersion roots:|                                        |
c+--------------------------+                                       !|
          Call Find_polynomial_roots(Q,ur,ui,N_Fields,ifail)        !|
          if (ifail.ne.0)       return                              !|
                                                                    !|
c+--------------------------+                                        |
...
c|Number of Im's:     |                                              |
c+--------------------+                                              |
          Call Check_Im_u_count(ui,N_Fields,N_Fields/2,ifail)       !|
          if (ifail.ne.0) return                                    !|
                                                                    !|
          do      i=1,N_Fields  !==========+                         |
        ...
        elseif (Magnetic.eq.2)  Then  !------------------------------|
                                                                    !|
c Special case of magnetic layer: M||Y, de-coupled sigma & pi:       |
c-------------------------------------------------------------       |
          sy    = g0*g0 + x_ij(2,2)                                 !|
          sz    = g0*g0 + x_ij(3,3)                                 !|
c         write (3,'(1001g14.7)') (g0*g0+x_ij(1,1)), sy, sz         !|
          xx    = 1. + x_ij(1,1)*xabs                               !|
          zz    = xx                                                !|
          delta = -xabs*x_ij(1,3)*x_ij(3,1)/xx                      !|
          u(1)  = Sqrt(sy)                                          !|
          u(2)  = Sqrt(sz+delta)                                    !|
