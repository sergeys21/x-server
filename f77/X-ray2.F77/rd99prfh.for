c This file is called from gid_sl99, gid_slm1 and gid_slm5.
c As compared to the previous version Rd97prfH.for,
c it adds options for the Henke & Cowan tables.
c
...
c  -- added October-1997:
     ...
     *                    progname, progrezv, istackrezv, ErrorFile
        if (istackrezv .lt. 10) Then  !----+
          ...
          progname = 'Read_Top99_0h'      !|
        else !-----------------------------+
          iirezv = 0                      !|
        endif  !---------------------------+
        lpn = Len_Trim (progname)
c -------------------------------------------------------
        lyr = 0                                 !make GNU compiler happy
        ifail = 0
        Thickness_Top = 0.
        TTT           = 0.
...
c=======================================================
        ...
        Subroutine Read_Sub99_0h (Code, W0, Wh,
     *                            Daa, Sigma,
     *                            wrk, line, *, *)

        Real*4          Sigma, Daa, W0, Wh,
     *                  Tmp(2)
        Integer         lpn, i, j, n,
     *                  iirezv, line,
     *                  iexit
