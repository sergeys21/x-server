...
c      +------------------------------------------------------+
c      | This version makes use of hard x-rays approximation  |
c      | - see the paper with Sinha.                          |
c      +------------------------------------------------------+
c
c                         By S.A.Stepanov
c
...
c
c '98    The same as '99.1b; simply renamed in order to distinguish
c        from mag_sl99 which was expanded to work for soft X-rays
c        while this version is an approximate solution for hard x-rays.
c '98.1  Derived from ter_sl97 and gid_sl97: script language for
c '98.1b The input files are made compatible with versions '99.2--'99.4
c '98.1c Added printouts of x_ij
c '98.3  Added Henke/Cowan option for X0h database
c '98.4  Added option to scan over qz
c '98.5  Added initializing variables (zeroing them) for GNU Fortran
c      +------------------------------------------------------+
c      | This version makes use of hard x-rays approximation  |
c      | (see the paper with Sinha)                           |
c      +------------------------------------------------------+
...
c cc    Parameter       (N_Tran_Max    = 101)           !50
        ...
        Complex*16      xF_ij(3,3),
     *                  E0(2), Es(2),
     *                  Ds(4, N_TT_Max),      ! sigma-Fields in layers
     *                  Dp(4, N_TT_Max),      ! pi-Fields in layers
...
c verified the output filename):
  ...
  111   format  ('MAG_sl: X-Ray Total Reflection from Magnetic ',
     ...
     *  'By S.Stepanov <sstepanov@anl.gov>',5x,
c    *  ' Version: 98.1  (hard r-rays), May, 2000')     !the same
c    *  ' Version: 98.1b (hard r-rays), Aug, 2004')     !the same
c    *  ' Version: 98.2  (hard r-rays), Nov, 2006')     !the same
c    *  ' Version: 98.3  (hard r-rays), Sep, 2012')     !the same
     *  ' Version: 98.5  (hard r-rays), Nov, 2013')     !the same
...
c-----------------------
c       txt(7) = 'x-ray wavelength/energy (range=[0.01-1000.0]Angstrom)'
c This '98 version is for hard x-rays only (6kev or 2A limit):
        txt(7) = 'x-ray wavelength/energy (range=[0.01-2.0]Angstrom)'
        Call    LineLoop (2,line,wrk,*100)
        Wave = 0.
        if (ixway .lt. 3) Then  !---------------------+
...
c This '98 version is for hard x-rays only:           |
c (6kev or 2A limit)                                  |
c         if (Wave .gt. 1000.)    goto 101           !|
          if (Wave .gt. 2.)       goto 101           !|
        endif  !--------------------------------------+
c-----------------------
...
c adn = means "Atomic Density" (in 1/cm^3)                   |
        ...
        Identical(N_Total) = 0
c N_TT=N_Top+2 -- vacuum+layers+substrate:
...
c Over columns or lines???                                  | |      |This only
              xF_ij(i,j)    = C8*Mvect(i,lyr)*Mvect(j,lyr) !| |      |inverts the
c             xF_ij(j,i)    = C8*Mvect(i,lyr)*Mvect(j,lyr) !| |      |difference
     /                      / (MvLen*MvLen)                !| |      |between E+
              do m=1,3  !============================+      | |      |and E- circ.
c Over columns or lines???                           |      | |      |polarizations!
                xF_ij(i,j)    = xF_ij(i,j)          !|      | |      |
c               xF_ij(j,i)    = xF_ij(j,i)          !|      | |      |
     -                        - im1*B8*Levi(i,j,m)  !|      | |      |
     *                        * Mvect(m,lyr)/MvLen  !|      | |      |
              enddo  !===============================+      | |      |
            enddo  !========================================+ |      |
            xF_ij(i,i) = xF_ij(i,i) + A8                     !|      |
          enddo  !============================================+      |
                                                                    !|
...
c 1=k=k0=x, 2=s=sigma=y, 3=p=pi=z_internal:                          |
          ...
          do i=1,2  !=====================================+          |
            do j=1,2  !================================+  |          |
              x_ij(i,j,lyr) = xF_ij(i+1,j+1)          !|  |          |
              x4 = sngl(Abs(x_ij(i,j,lyr)))           !|  |          |
c Find the maximum magnetic contribution:              |  |          |
              if (x4 .gt. xm_max(lyr)) xm_max(lyr)=x4 !|  |          |
            enddo  !===================================+  |          |
c           x_ij(i,i,lyr) = xF_ij(i,i) + x0(lyr)         !|v.1'0 bug?|
            x_ij(i,i,lyr) = xF_ij(i+1,i+1) + x0(lyr)     !|          |
          enddo  !========================================+          |
          ...
          if (xm_max(lyr) .gt. (1.E-5)) Then !--------------------+  |
            ...
            Magnetic(lyr) = 0                    !non-magnetic    |  |
                                                                    ...
                                                                    !|
c         if (lyr .eq. 1) Then !---------------------+               |
c           write (3,127) lyr, "Vacuum          ",  !|               |
c    *                    Magnetic(lyr),            !|               |
c    *                    xm_max(lyr)               !|               |
c         else !-------------------------------------|               |
c           write (3,127) lyr, Code(1,lyr-1),       !|               |
c    *                    Magnetic(lyr),            !|               |
c    *                    xm_max(lyr)               !|               |
c         endif  !-----------------------------------+               |
c 127     format(' ----------'/1x,i6,2x,a,'Magnetic=',i1,
c    *           '  x_mag/x0=',g12.3)                               !|
c         write (3,128) ((x_ij(i,j,lyr),i=1,2),j=1,2)               !|
c 128     format(3(1x,2('  (',g11.5,',',g11.5,')  ')/))             !|
                                                                    !|
        enddo  !=====================================================+
