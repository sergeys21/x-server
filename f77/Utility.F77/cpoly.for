...
C
C     LAPACK ALGORITHM 419 COLLECTED ALGORITHMS FROM ACM.
...
C
C ADAPTED FOR GNU FORTRAN BY SERGEY STEPANOV 2018/08/10
C
...
C COMMON AREA
     ...
     *         PR(50),PI(50),HR(50),HI(50),QPR(50),QPI(50),
     *         QHR(50),QHI(50),SHR(50),SHI(50)
...
C THE DIMENSION OF THE ARRAYS IN THE COMMON AREA.
     ...
     *         OPR(1+DEGREE),OPI(1+DEGREE),ZEROR(DEGREE),ZEROI(DEGREE),
     *         CMOD,SCALFAC,CAUCHY
        REAL*8 DSQRT
        LOGICAL FAIL,CONV
        INTEGER DEGREE,CNT1,CNT2,I,NN,IDNN2
...
c       IF (OPR(1) .NE. 0.0D0 .OR. OPI(1) .NE. 0.0D0) GO TO 10
        IF (ABS(OPR(1)).GT.SMALNO .OR. ABS(OPI(1)).GT.SMALNO) GO TO 10
            FAIL = .TRUE.
            RETURN
C REMOVE THE ZEROS AT THE ORIGIN IF ANY.
  10    CONTINUE
c       IF (OPR(NN) .NE. 0.0D0 .OR. OPI(NN) .NE. 0.0D0) GO TO 20
        IF (ABS(OPR(NN)).GT.SMALNO .OR. ABS(OPI(NN)).GT.SMALNO) GO TO 20
            IDNN2 = DEGREE-NN+2
            ZEROR(IDNN2) = 0.0D0
            ZEROI(IDNN2) = 0.0D0
            NN = NN-1
...
C SCALE THE POLYNOMIAL.
        BND = SCALFAC(NN,SHR,ETA,INFIN,SMALNO,BASE)
c       IF (BND .EQ. 1.0D0) GO TO 40
        IF (ABS(BND-1.0D0).LT.ETA) GO TO 40
        DO 35 I = 1,NN
            PR(I) = BND*PR(I)
            PI(I) = BND*PI(I)
  35    CONTINUE
...
C COMMON AREA
     ...
     *         PR(50),PI(50),HR(50),HI(50),QPR(50),QPI(50),
     *         QHR(50),QHI(50),SHR(50),SHI(50)
        REAL*8 ZR,ZI,MP,MS,OMP,RELSTP,R1,R2,CMOD,ERREV,TP
        REAL*8 DSQRT
        LOGICAL CONV,B,BOOL
        INTEGER L3,NN,I,J
        CONV = .FALSE.
...
C PLACING THE PARTIAL SUMS IN Q AND THE COMPUTED VALUE IN PV.
        IMPLICIT NONE
        REAL*8 PR(NN),PI(NN),QR(NN),QI(NN),
     *         SR,SI,PVR,PVI,T
        INTEGER NN,I
        QR(1) = PR(1)
        QI(1) = PI(1)
...
C POLYNOMIAL - PT IS THE MODULUS OF THE COEFFICIENTS.
        IMPLICIT NONE
        REAL*8 Q(NN),PT(NN),X,XM,F,DX,DF
        REAL*8 ETA,INFIN,SMALNO,BASE
...
c       IF (PT(N).EQ.0.0D0) GO TO 20
        IF (ABS(PT(N)).LT.SMALNO) GO TO 20
...
C DO NEWTON ITERATION UNTIL X CONVERGES TO TWO DECIMAL PLACES.
  50    CONTINUE
        IF (DABS(DX/X) .LE. .005D0) GO TO 70
            Q(1) = PT(1)
            DO 60 I = 2,NN
                Q(I) = Q(I-1)*X+PT(I)
  60        CONTINUE
...
C=======================================================================
        REAL*8 FUNCTION SCALFAC(NN,PT,ETA,INFIN,SMALNO,BASE)
...
C FIND LARGEST AND SMALLEST MODULI OF COEFFICIENTS.
        ...
        DO 10 I = 1,NN
            X = PT(I)
            IF (X .GT. VMAX) VMAX = X
c           IF (X .NE. 0.0D0 .AND. X .LT. VMIN) VMIN = X
            IF (ABS(X).GT.SMALNO .AND. X.LT.VMIN) VMIN = X
  10    CONTINUE
C SCALE ONLY IF THERE ARE VERY LARGE OR VERY SMALL COMPONENTS.
        SCALFAC = 1.0D0
        IF (VMIN .GE. LO .AND. VMAX .LE. HI) RETURN
        X = LO/VMIN
        IF (X .GT. 1.0D0) GO TO 20
            SC = 1.0D0/(DSQRT(VMAX)*DSQRT(VMIN))
            GO TO 30
  20    CONTINUE
        SC = X
        IF (INFIN/SC .GT. VMAX) SC = 1.0D0
  30    CONTINUE
        L = INT(DLOG(SC)/DLOG(BASE) + .500)
        SCALFAC = BASE**L
        RETURN
        END
...
C COMPLEX DIVISION C = A/B, AVOIDING OVERFLOW.
        IMPLICIT NONE
        REAL*8 AR,AI,BR,BI,CR,CI,R,D
        REAL*8 ETA,INFIN,SMALNO,BASE
        REAL*8 DABS
        CALL MCON(ETA,INFIN,SMALNO,BASE)
c       IF (BR .NE. 0.0D0 .OR. BI .NE. 0.0D0) GO TO 10
        IF (ABS(BR).GT.SMALNO .OR. ABS(BI).GT.SMALNO) GO TO 10
C DIVISION BY ZERO, C = INFINITY.
            CR = INFIN
            CI = INFIN
            RETURN
  10    CONTINUE
        IF (DABS(BR) .GE. DABS(BI)) GO TO 20
            R = BR/BI
            D = BI+R*BR
            CR = (AR*R+AI)/D
            CI = (AI*R-AR)/D
