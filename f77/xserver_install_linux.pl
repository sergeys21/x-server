...
# use warnings::unused;
  ...
  my ($dest_x0h, $dirs_prefix, $dirs_postfix);
  my (@dirs_x0h, $ndirs, $i, @exefiles, $exefile, $exe, $script);
  ...
  if (defined $ENV{X0H}) {
     ...
     die 'Please define X0H environment to point it to the x0h directory.'."\n";
  }
  print STDOUT '       X0H directory: '.$dest_x0h."\n";
  print STDOUT 'Codeblocks directory: '.$dirs_prefix."\n";
  if (! -e $dest_x0h)    {die '*** X0H directory "'.$dest_x0h.'" does not exist.';}
  if (! -e $dirs_prefix) {die '*** Codeblocks directory "'.$dirs_prefix.'" does not exist.';}
